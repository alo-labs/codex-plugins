...
```
as a user-instruction Claude should write to `.codex/settings.local.json`. This is an instruction to persistently disable all permission checks for Claude Code tool calls in that project. While this is documented as for "isolated environments," the instruction is presented without mandatory isolation verification, and could lead users in non-isolated environments to permanently disable all permission guardrails.
