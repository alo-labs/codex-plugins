...
# PreToolUse hook — agentmemory capture gate before substantive work (opt-in enforced).
# Active only when recommended_tools.agentmemory.enabled_by_user is true.
...
emit_block() {
  ...
  sb_hook_audit_record "agentmemory-gate" "$hook_event" "deny" "$reason" "${file_path:-${command_str:-}}" 2>/dev/null || true
...
if [[ -n "$file_path" ]]; then
  if sb_agentmemory_edit_path_is_exempt "$file_path" "$config_file"; then
    exit 0
  fi
elif [[ -n "$command_str" ]]; then
  if sb_agentmemory_command_is_exempt "$command_str"; then
    exit 0
  fi
...
if ! sb_agentmemory_cli_available; then
  emit_block "$(sb_agentmemory_block_message_no_cli "$config_file")"
  exit 0
fi
...
if sb_agentmemory_graphify_synergy_active "$config_file"; then
  exit 0
fi

if sb_agentmemory_usage_is_fresh "$config_file"; then
  exit 0
fi

ttl="$(sb_agentmemory_usage_ttl_seconds "$config_file")"
emit_block "$(sb_agentmemory_block_message_stale_usage "$ttl")"
exit 0
