#!/usr/bin/env bash
# Channel-timeline monitor for Codex delegation logs (parent supervision).
set -euo pipefail

LOG_FILE=""
INTERVAL="${AGENT_CODEX_MONITOR_INTERVAL:-30}"
STALL_SEC="${AGENT_CODEX_MONITOR_STALL_SEC:-300}"
ONCE=0
...
emit_bullet() {
  printf '[agent-codex monitor] %s — %s\n' "$(date -u +%H:%M:%SZ)" "$1"
}
...
scan_log() {
  ...
  if [[ "$prompt_seen" -eq 0 ]] && grep -qE 'prompt\.submitted|Prompt submitted|Submitted prompt' <<<"$content" 2>/dev/null; then
  ...
  if grep -qiE 'ERROR:|0-token|rate limit|429|token plan|hooks need review|not logged in' <<<"$content"; then
    emit_bullet "• signal: $(grep -oiE 'ERROR:[^[:space:]]+|0-token|rate limit|429|token plan|hooks need review' <<<"$content" | tail -1 || echo anomaly)"
  fi

  floor="${SB_AGENT_CODEX_LOG_FLOOR:-512}"
