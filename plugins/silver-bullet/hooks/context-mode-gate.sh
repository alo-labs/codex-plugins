...
# PreToolUse hook — Context Mode install/wiring gate (opt-in enforced).
# Active only when recommended_tools.context_mode.enabled_by_user is true.
# Usage (ctx_* vs Read) is instructional in silver-bullet.md §2g-ii — not hook-blocked.
...
[[ -f "$_lib_dir/context-mode-gate.sh" ]] && source "$_lib_dir/context-mode-gate.sh"
...
emit_block() {
  ...
  sb_hook_audit_record "context-mode-gate" "$hook_event" "deny" "$reason" "${file_path:-${command_str:-}}" 2>/dev/null || true
...
if [[ -n "$file_path" ]]; then
  if sb_context_mode_edit_path_is_exempt "$file_path" "$config_file"; then
    exit 0
  fi
elif [[ -n "$command_str" ]]; then
  if sb_context_mode_command_is_exempt "$command_str"; then
    exit 0
  fi
...
if ! sb_context_mode_node_ok "$config_file"; then
  emit_block "$(sb_context_mode_block_message_node "$config_file")"
  exit 0
fi

if ! sb_context_mode_cli_available; then
  emit_block "$(sb_context_mode_block_message_no_cli "$config_file")"
  exit 0
fi

if ! sb_context_mode_platform_artifact_present "$project_root"; then
  emit_block "$(sb_context_mode_block_message_no_platform "$config_file")"
  exit 0
fi

if ! sb_context_mode_instruction_fragment_present "$project_root"; then
  emit_block "$(sb_context_mode_block_message_no_fragment)"
  exit 0
fi
