...
|---|---|---|
| `shell` + `fileWrite` | HIGH | Hooks write to `$HOME/.codex/.silver-bullet/` state files and `docs/sessions/` session logs. Scope is validated to `$HOME/.codex/` prefix. |
...
- State file paths are validated to stay within `$HOME/.codex/` prefix — prevents path traversal.
...
1. Tracking skill invocations in a state file (`$HOME/.codex/.silver-bullet/state`)
...
- **File system read:** Source files (via src_pattern), `.planning/` directory, `docs/`, `$HOME/.codex/.silver-bullet/` state directory
- **File system write:** `$HOME/.codex/.silver-bullet/` (state, mode, session-start-time, branch, call-count, etc.), `docs/sessions/` (session logs), `.planning/SPEC.md` (PR traceability), `.planning/.context-cache/` (semantic compression)
...
│                 to $HOME/.codex/ prefix.                         │
│ Attack Vector : 1. Attacker gains write access to             │
│                    .silver-bullet.json                        │
│                 2. Sets state_file to path outside $HOME/.codex/  │
...
│                 default $HOME/.codex/.silver-bullet/state        │
...
│                 "TIMEOUT" to $HOME/.codex/.silver-bullet/timeout │
...
**FINDING-5.1 (HIGH, 7.0):** Could a reasonable reviewer rate this lower? YES — the actual exploitability is LOW because path validation constrains all file writes to `$HOME/.codex/`, umask prevents world-readable files, and symlink rejection prevents indirection attacks. However, the severity floor for tool-scope escalation (7.0) applies. The floor is correctly enforced.
...
- **State file:** `$HOME/.codex/.silver-bullet/state` — tracks which workflow skills have been invoked
