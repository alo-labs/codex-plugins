...
| State file tampering | `$HOME/.codex/.silver-bullet/state` is append-only; session-start clears stale markers | State mgmt |
...
| Stale quality gate markers | Session-start hook clears `$HOME/.codex/.silver-bullet/quality-gate-state` | Session lifecycle |
