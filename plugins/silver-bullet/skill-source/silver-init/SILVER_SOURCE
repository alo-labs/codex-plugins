...
### 1.1aФ1.1f Recommended tools (opt-in)

Graphify, agentmemory, RTK, and Context Mode each require explicit user consent before install
and hook enforcement. **Do not auto-install.**

**Load and follow:** `references/recommended-tools-opt-in.md` (consent prompts, install steps,
suspension handling, stack optimizer). Per-host commands: `scripts/lib/host-install-guides/<SB_HOST>.md`
after detecting runtime via `hooks/lib/runtime-paths.sh` or `SILVER_BULLET_RUNTIME`.
