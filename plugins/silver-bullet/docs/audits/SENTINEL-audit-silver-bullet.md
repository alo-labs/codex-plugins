...
> - FINDING-5.1 (world-readable `/tmp/` state files): **REMEDIATED** — all state migrated to `$HOME/.codex/.silver-bullet/` with `umask 0077`.
...
| `session-log-init.sh` (sentinel) | process spawn, file write to `$HOME/.codex/` | LOW — legitimate use, scoped to user dir |
...
1. **State files in `$HOME/.codex/.silver-bullet/`** — user-owned, 0077 umask. Mitigated vs prior audit.
...
7. **Sentinel process** — background `sleep` process writes "TIMEOUT" to `$HOME/.codex/.silver-bullet/timeout`. PID stored and killed on next session start.
...
│                 ($HOME/.codex/plugins/cache/*/silver-bullet/)  │
