...
# PreToolUse hook — Alumnium MCP wiring gate (opt-in enforced).
# Active only when recommended_tools.alumnium.enabled_by_user is true.
...
[[ -f "$_lib_dir/alumnium-gate.sh" ]] && source "$_lib_dir/alumnium-gate.sh"
...
emit_block() {
  ...
  sb_hook_audit_record "alumnium-gate" "$hook_event" "deny" "$reason" "${file_path:-${command_str:-}}" 2>/dev/null || true
...
sb_alumnium_required "$config_file" || exit 0
...
if ! sb_alumnium_npx_available; then
  emit_block "$(sb_alumnium_block_message_no_mcp "$config_file")"
  exit 0
fi

if ! sb_alumnium_platform_artifact_present "$project_root"; then
  emit_block "$(sb_alumnium_block_message_no_mcp "$config_file")"
  exit 0
fi
