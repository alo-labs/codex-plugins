...
emit_block() {
  ...
  sb_hook_audit_record "graphify-gate" "$hook_event" "deny" "$reason" "${file_path:-${command_str:-}}" 2>/dev/null || true
...
sb_graphify_required "$config_file" || exit 0
...
if [[ -n "$file_path" ]]; then
  if sb_graphify_edit_path_is_exempt "$file_path" "$config_file"; then
    exit 0
  fi
...
if ! sb_graphify_index_exists "$project_root" "$config_file"; then
  emit_block "$(sb_graphify_block_message_no_index "$graph_rel")"
  exit 0
fi
...
emit_block "$(sb_graphify_block_message_stale_query "$graph_rel" "$ttl")"
exit 0
