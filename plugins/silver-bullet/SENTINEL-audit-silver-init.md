...
| Tool | Usage | Risk |
|------|-------|------|
| **Bash** | 12+ invocations: `test -f`, `command -v`, `basename`, `git remote`, `ls -d`, `mkdir -p`, `git add`, `git commit`, `touch`, `ls $HOME/.codex/...` | HIGH — shell access |
...
| **Glob** | Search for plugin skill files in `$HOME/.codex/plugins/cache/` | LOW — read-only |
...
4. **Plugin cache directory** — `$HOME/.codex/plugins/cache/*/...` (globbed for dependency detection)
...
- **File system read**: Project root files, `$HOME/.codex/` directory, docs/ directory
...
Credential file targeting: The skill reads `$HOME/.codex/plugins/cache/` and `$HOME/.codex/commands/` (Phase 1.2-1.5, Phase 2.7) — these are plugin directories, not credential files. It also reads `.codex/settings.json` (Phase 1.6) — this is a settings file, not a credential file. No reads target `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.npmrc`, or other credential-storing paths.
